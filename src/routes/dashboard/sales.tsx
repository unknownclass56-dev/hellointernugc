...
function SalesDashboard() {
  ...
  useEffect(() => {
    ...
    // Fetch transactions linked to this rep for chart
  ...
  const handleSaveStudentNote = async () => {
    ...
    try {
      ...
      // Optional logic: if they completed a course/internship sale, create enrollment & transaction records
  ...
  return (
    <div className="p-6 space-y-8 bg-[#0b1329] min-h-screen text-slate-100 font-sans">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-800 pb-6">
        <div>
          ...
          <h1 className="text-3xl font-extrabold uppercase tracking-tight text-white">
            Welcome Back, <span className="text-blue-400">{user?.user_metadata?.full_name || "Sales Rep"}</span>
          </h1>
        ...
        <div className="bg-slate-900/80 px-4 py-3 rounded-2xl border border-slate-800 shadow-xl flex items-center gap-3">
          <div className="size-10 rounded-xl bg-blue-500/10 text-blue-400 grid place-items-center font-black">
          ...
          <div>
            ...
            <div className="text-sm font-extrabold text-slate-200">{user?.email}</div>
          </div>
        </div>
      </div>
      ...
      {/* Leads / Student detail lists */}
      <Card className="bg-slate-900/60 border border-slate-800 shadow-2xl overflow-hidden">
